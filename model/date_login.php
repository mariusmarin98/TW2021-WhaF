...
if ( $_SERVER['REQUEST_METHOD'] == 'POST' ) {
    ...
    //INCARCAM FIECARE INPUT DIN HTML IN VARIABILE PHP PRIN METODA POST
                                ...
                                if ( strlen( $parola )>7 ) {
                                    if ( preg_match( '/[A-Z]/', $parola ) ) {
                                        ...
                                        if ( ctype_alpha( $prenume ) ) {
                                            ...
                                            $stmt = $conn->prepare( 'SELECT `email` FROM `utilizatori` WHERE `email` = ?' );
                                            ...
                                            if ( $nr_rows == 0 ) {
                                                ...
                                                $sql = 'INSERT INTO utilizatori (nume, prenume, email, parola, sex, data_nasterii) VALUES ( ?, ?, ?, ?, ?, ?);';
                                                $stmt = $conn->prepare( $sql );
                                                $stmt->bind_param( 'ssssss', $nume, $prenume, $email, $parola_hash, $sex, $data_nasterii );
